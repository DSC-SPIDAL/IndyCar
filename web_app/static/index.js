...
let canvasHeight = (shortStraightWay.length + (2 * scaledRadius) + shortStraightWay.width + 80);
var draw = SVG('drawing').size(window.innerWidth, canvasHeight);
...
console.log("H", draw.node.clientHeight, shortStraightWay.length + (2 * scaledRadius) + shortStraightWay.width, draw);
//let canvasHeight = draw.node.clientHeight === 0 ? (shortStraightWay.length + (2 * scaledRadius) + shortStraightWay.width + 120) : draw.node.clientHeight;
let paddingTop = 40;//((canvasHeight) - shortStraightWay.length - (2 * scaledRadius) - shortStraightWay.width) / 2;
let paddingLeft = (window.innerWidth - longStraightWay.length - 2 * scaledRadius - 0.5 * longStraightWay.width) / 2;
...
let path = draw.path
    ...
    .center(window.innerWidth / 2, (canvasHeight) / 2);
...
//start line
let startLine = draw.rect(20 * scale, longStraightWay.width).fill(pattern).move(x2, y1);
...
function setSpeedInformation(speed_arr, delay_time, order) {
    setTimeout(function () {
        ...
        for (i = 0; i < speed_arr.length; i++) {
            ...
            $('.speed-data-rank-' + order + ' .speed-data-lap-section-time').eq(i).text(speed_arr[i].toFixed(1))
        }
    }, delay_time);
}
...
function animateCar(image, animationTimesArrAll, lengthPropArrAll, startPosArrAll, y) {
    console.log(image, animationTimesArrAll, lengthPropArrAll, startPosArrAll, y)
    var carContainer = draw.group();
    var car = carContainer.image(image).size(5.5 * carScale, 2.5 * carScale).hide();
...
function setWeatherInformation(weatherInfo, delay) {
    ...
    setTimeout(function () {
        $(".weather-indicator-ambient-temp").text(weatherInfo.ambient_temp)
        $(".weather-indicator-pressure").text(weatherInfo.barometric_pressure)
        $(".weather-indicator-humidity").text(weatherInfo.relative_humidity + "%")
        $(".weather-indicator-time").text(weatherInfo.time_of_day)
    }, delay)
}
...
function setLapInformation_Outer(animationTimesArrAll) {
    ...
    for (g = 1; g < animationTimesArrAll.length; g++) {
        ...
        delay = prevDelay + timeArr.reduce(function (acc, val) {
...
function setEntryInformation(carnum, order) {
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.speed-data-rank-' + order + ' .speed-data-driver-info-other-licence').text(entryinfo.license)
        },
        type: 'GET'
    });
}
...
function setRankforCar(order, rank, delay, car_num) {
    setTimeout(function () {
        $('.speed-data-rank-' + order + ' .speed-data-driver-info-other-rank').text(rank)
        $.ajax({
...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.event-name').text(eventinfo)
            $('.run-name').text(runinfo)
            $('.start-date').text(timeinfo)
        },
        type: 'GET'
    });
